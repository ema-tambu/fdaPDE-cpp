...
template <typename Model, typename RegularizationType> 
void RegressionBase<Model, RegularizationType>::set_dirichlet_bc(SparseBlockMatrix<double, 2, 2>& A, DVector<double>& b) {
    std::size_t n = A.rows() / 2;

    auto boundary_dofs_Dirichlet_ = this->pde().matrix_bc_Dirichlet();
    auto boundary_data_Dirichlet_ = this->pde().dirichlet_boundary_data();
